...
{
public:
    string helper(const vector<pair<string, int>> &vals, int timestamp)
    {
        int hi = vals.size() - 1;
        int lo = 0;
        int mid;
        ...
        if (hi >= 0)
        {
            return vals[hi].first;
        }
        return "";
    }

    unordered_map<string, vector<pair<string, int>>> hm;
    TimeMap()
    {
    }
    ...
    {
        hm[key].push_back({value, timestamp});
    }
    ...
    string get(string key, int timestamp)
    {
        ...
        return helper(hm[key], timestamp);
    }
};
